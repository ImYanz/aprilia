...
╚═════════════════════════╝
          ...
          if (botSpam.antispam && m.text && user && user.lastCommandTime && (Date.now() - user.lastCommandTime) < 5000 && !isROwner) {
	        ...
	        const hl = _prefix;
        const adminMode = global.db.data.chats[m.chat].modeadmin;
...
export async function callUpdate(callUpdate) {
  ...
  for (const nk of callUpdate) {
    if (nk.isGroup == false) {
      if (nk.status == 'offer') {
        const callmsg = await this.reply(nk.from, `hai *@${nk.from.split('@')[0]}*, ${nk.isVideo ? 'vidcall' : 'call'} tidak diizinkan, Anda akan diblokir.\n-\nJika Anda secara tidak sengaja menelepon, hubungi pencipta saya untuk membuka blokir Anda!`, false, {mentions: [nk.from]});
         let data = global.owner.filter(([id, isCreator]) => id && isCreator)
         await this.sendContact(nk.from, data.map(([id, name]) => [id, name]), false, { quoted: callmsg })
       // const vcard = `BEGIN:VCARD\nVERSION:3.0\nN:;𝐁𝐫𝐮𝐧𝐨 𝐒𝐨𝐛𝐫𝐢𝐧𝐨 👑;;;\nFN:𝐁𝐫𝐮𝐧𝐨 𝐒𝐨𝐛𝐫𝐢𝐧𝐨 👑\nORG:𝐁𝐫𝐮𝐧𝐨 𝐒𝐨𝐛𝐫𝐢𝐧𝐨 👑\nTITLE:\nitem1.TEL;waid=6281239303460:+62 8123 9303 460\nitem1.X-ABLabel:𝐁𝐫𝐮𝐧𝐨 𝐒𝐨𝐛𝐫𝐢𝐧𝐨 👑\nX-WA-BIZ-DESCRIPTION:[❗] ᴄᴏɴᴛᴀᴄᴛᴀ ᴀ ᴇsᴛᴇ ɴᴜᴍ ᴘᴀʀᴀ ᴄᴏsᴀs ɪᴍᴘᴏʀᴛᴀɴᴛᴇs.\nX-WA-BIZ-NAME:𝐁𝐫𝐮𝐧𝐨 𝐒𝐨𝐛𝐫𝐢𝐧𝐨 👑\nEND:VCARD`;
...
global.dfail = (type, m, conn) => {
  const msg = {
    rowner: '*This command is only for bot owners*',
    owner: '*This command is only for bot owners.*',
    mods: '*This command is only for mods users.*',
    premium: '*This command is only for premium users.*',
    group: '*This command is for groups only.*',
    private: '*This command is for ptivat only.*',
    admin: '*This command is for admin only.*',
    botAdmin: '*This command can be used once the bot has become a group admin.*',
    unreg: '*You are not registered yet, please register first*\n*How to register:*\n\t*.reg name.age*\n*Example: .reg yanz.16*',
    restrict: '*Restrictions have not been activated.*',
  }[type];
  const aa = {quoted: m, userJid: conn.user.jid};
  const prep = generateWAMessageFromContent(m.chat, {extendedTextMessage: {text: msg, contextInfo: {externalAdReply: {title: yanz, body: yanz1, thumbnail: imagen1, sourceUrl: ''}}}}, aa);
